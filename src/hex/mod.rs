...
impl ToolGroup for HexTools {
    ...
    async fn call(&self, name: &str, args: Value) -> ToolResult<ToolOutput> {
        ...
        .await
        .map_err(|e| ToolFailure::Failed(format!("hex task failed: {e}")))?
    }
}

fn view(arguments: &Value, ctx: &Policy) -> ToolResult<ToolOutput> {
    ...
    let mut file = std::fs::File::open(&path)
        .map_err(|e| ToolFailure::Failed(format!("could not open {}: {e}", path.display())))?;
    let total = file
        .metadata()
    ...
    file.seek(SeekFrom::Start(offset))
        .map_err(|e| ToolFailure::Failed(format!("seek failed: {e}")))?;
    let to_read = requested.min(total - offset);
    let mut buffer = vec![0u8; usize::try_from(to_read).unwrap_or(0)];
    file.read_exact(&mut buffer).map_err(|e| ToolFailure::Failed(format!("read failed: {e}")))?;
...
fn patch(arguments: &Value, ctx: &Policy) -> ToolResult<ToolOutput> {
    let path = ctx.resolve(args::string(arguments, "path")?)?;
    ...
    if apply {
        ...
        std::fs::write(&backup, &original)
            .map_err(|e| ToolFailure::Failed(format!("could not write backup: {e}")))?;
...
fn substitute(haystack: &[u8], needle: &[u8], replacement: &[u8]) -> ToolResult<(Vec<u8>, String)> {
    if needle.is_empty() {
        return Err(ToolFailure::InvalidArguments("`search_hex` must not be empty".into()));
    }
...
fn overwrite(original: &[u8], offset: u64, data: &[u8]) -> ToolResult<(Vec<u8>, String)> {
    let start = usize::try_from(offset)
        .map_err(|_| ToolFailure::InvalidArguments(format!("offset {offset} is too large")))?;
...
fn decode(text: &str, field: &str) -> ToolResult<Vec<u8>> {
    ...
    hex::decode(&cleaned)
        .map_err(|e| ToolFailure::InvalidArguments(format!("{field:?} is not valid hex: {e}")))
}
