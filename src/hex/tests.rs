...
impl Fixture {
    ...
    fn ctx(&self, mutable: bool) -> Policy {
        Policy::new(mutable, false, std::slice::from_ref(&self.root), &[], 64 * 1024 * 1024)
    }
}

async fn run(name: &str, arguments: Value, ctx: &Policy) -> ToolResult<Value> {
    let result = HexTools::new(ctx.clone()).call(name, arguments).await?;
    Ok(result.structured.expect("hex tools return structured results"))
}
...
async fn malformed_hex_is_an_argument_error() {
    ...
    for bad in ["zz", "abc"] {
        ...
        .await
        .unwrap_err();
        assert!(matches!(err, ToolFailure::InvalidArguments(_)), "{bad} should be rejected");
    }
}
