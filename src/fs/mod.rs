...
impl ToolGroup for FsTools {
    ...
    async fn call(&self, name: &str, args: Value) -> ToolResult<ToolOutput> {
        ...
        tokio::task::spawn_blocking(move || request.run(&ctx))
            .await
            .map_err(|e| ToolFailure::Failed(format!("search task failed: {e}")))?
    }
}
...
impl SearchRequest {
    fn parse(arguments: &Value, ctx: &Policy) -> ToolResult<Self> {
        ...
        if pattern.is_empty() {
            return Err(ToolFailure::InvalidArguments("pattern must not be empty".into()));
        }
        ...
        let matcher = if args::bool_or(arguments, "is_regex", false)? {
            ...
            Matcher::Regex(builder.build().map_err(|e| {
                ToolFailure::InvalidArguments(format!("invalid regular expression: {e}"))
            })?)
        } else {
    ...
    fn run(&self, ctx: &Policy) -> ToolResult<ToolOutput> {
    ...
    fn candidate_files(&self, ctx: &Policy) -> Vec<PathBuf> {
        if self.root.is_file() {
            // `max_filesize` below only governs the walk, so a directly named
            // file would otherwise bypass the cap entirely.
            return match ctx.check_size(&self.root) {
                Ok(_) => vec![self.root.clone()],
                Err(_) => Vec::new(),
            };
        }
