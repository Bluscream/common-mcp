...
fn mutable_ctx(root: &Path) -> Policy {
    Policy::new(true, false, &[root.to_path_buf()], &[], 64 * 1024 * 1024)
}

fn readonly_ctx(root: &Path) -> Policy {
    Policy::new(false, false, &[root.to_path_buf()], &[], 64 * 1024 * 1024)
}

async fn search(arguments: Value, ctx: &Policy) -> ToolResult<Value> {
    let result = FsTools::new(ctx.clone()).call("grep_search", arguments).await?;
    Ok(result.structured.expect("search results are structured"))
}
